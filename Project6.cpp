...
// comments: Q1: Yes, Q2: Yes, Q3: It's not the smae as mine in proj5 but I understand it.
...
void print_calendar(int month, int day, int year)
{
    ...
    if (!isValid(month, day, year))
    { // if the date is NOT valid, print out error message and return
...
{
    if (!isValid(year))
    { // if the year is NOT valid, print out error message and return
...
bool print_calendar(int year, string fileName)
{
    ...
    cout << "\n\tThe calendar of year " << year << " is successfully output to file " << fileName << endl;
    // return true
    return true;
}
